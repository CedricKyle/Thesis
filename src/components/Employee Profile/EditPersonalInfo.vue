  ...
  if (employee.value) {
    formData.value = {
      ...
      date_of_birth: employee.value.date_of_birth
        ? new Date(employee.value.date_of_birth).toISOString().split('T')[0]
        : '',
...
const collectChanges = () => {
  ...
  // Show notification
  showNotification(
    'Your changes have been collected but NOT SAVED yet. Please click the "Save Changes" button on your profile page to save them permanently.',
    'success',
  )
...
<template>
  <div class="p-4">
    ...
    <h2 class="text-2xl font-bold mb-6 text-black">Edit Personal Information</h2>

    <!-- Info Message -->
    <div class="mb-6 p-3 bg-blue-50 border border-blue-200 rounded-md">
      <p class="text-blue-700 text-sm">
        <strong>Note:</strong> You can update your full name, contact number, and date of birth
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      <form @submit.prevent="collectChanges" class="grid gap-6">
        ...
        <div class="form-control">
          ...
          />
          <span v-if="errors.full_name" class="text-red-500 text-xs mt-1">{{
            errors.full_name
          }}</span>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-4">
          ...
          <div class="form-control">
            <label class="label">
              <span class="label-text text-gray-700"
                >Date of Birth <span class="text-red-500">*</span></span
              >
            </label>
            <input
            ...
            />
            <span v-if="errors.date_of_birth" class="text-red-500 text-xs mt-1">{{
        ...
        <div class="grid md:grid-cols-2 gap-4">
          <div class="form-control">
            ...
            />
            <span v-if="errors.contact_number" class="text-red-500 text-xs mt-1">{{
        ...
        <div class="flex justify-end gap-2">
          <button type="button" @click="goBack" class="btn btn-ghost" :disabled="isSaving">
            Cancel
          </button>
          <button type="submit" class="btn btn-primary bg-primaryColor" :disabled="isSaving">
